...
function jobCard(j) {
  ...
  if (compactMode && isTech) {
    ...
    return '<article class="job-card compact-card rounded-xl cursor-pointer active:opacity-90 overflow-hidden" data-id="' + esc(j.job_id) + '" style="' + distBar + '">' +
      '<div class="p-2 min-h-[88px] flex flex-col justify-between">' +
        '<div>' +
          '<div class="flex items-start justify-between gap-1">' +
            '<p class="font-semibold text-[12px] leading-tight line-clamp-2 text-slate-800">' + esc(j.client_name) + '</p>' +
            returnBadge +
          '</div>' +
          '<p class="text-[11px] font-medium text-slate-600 mt-1">' + esc(j.time || '—') + '</p>' +
        '</div>' +
